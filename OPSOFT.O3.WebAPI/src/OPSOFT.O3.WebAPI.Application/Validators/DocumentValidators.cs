...
public class CreateInStockRequestValidator : AbstractValidator<CreateInStockRequest>
{
    public CreateInStockRequestValidator()
    {
        RuleFor(x => x.Fsupplyid).NotEmpty().WithMessage("供应商不能为空");
        When(x => x.Ftypeid == 2, () =>
        {
        ...
        }).Otherwise(() =>
        {
            ...
            RuleForEach(x => x.Entries).ChildRules(e =>
            {
                ...
                e.RuleFor(m => m.Frealqty).GreaterThan(0).WithMessage("实收数量必须大于0");
            });
        });
    }
}
