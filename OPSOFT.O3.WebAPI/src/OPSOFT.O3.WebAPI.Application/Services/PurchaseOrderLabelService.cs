...
/// <summary>
/// 采购标签打印（采购订单）服务。
/// 列表：已审核(40)且非禁用的采购订单按明细行展开。
/// 条码生成：按"本次打印数量/包装数量"生成 N 个条码，写入 T_BD_BARCODERS(主档) + T_BD_BARCODERS1(源单档)。
/// 条码编号：走编码规则取号服务 IBillCodeService（SYS_BARCODE 规则 + SYS_BARCODENO 原子流水，
/// 种子默认规则 = yyyyMMdd + 6 位日流水，可在 系统管理→编码规则 调整）。
/// 条码状态/库存状态/打印来源等权威枚举见类内常量。
/// </summary>
public class PurchaseOrderLabelService : IPurchaseOrderLabelService
{
    ...
    private const string PrgKey = "LabelPurchaseOrder";
    ...
    public PurchaseOrderLabelService(
        ISqlSugarClient db,
    ...
    public async Task<PagedResult<PurchaseOrderLabelListDto>> GetPagedListAsync(PagedRequest request)
    {
        ...
        var hq = _db.Queryable<TPurPoOrder>().Where(h => !h.FDeleted && h.FStatus == 40 && !h.FDisabled);
        ...
        var query = _db.Queryable<TPurPoOrderEntry>().Where(e => !e.FDeleted && headerIds.Contains(e.FInterId));
        ...
        var headers = await _db.Queryable<TPurPoOrder>().Where(h => hids.Contains(h.Uid)).ToListAsync();
        ...
        var items = entries.Select(e =>
        {
            ...
            return new PurchaseOrderLabelListDto
            {
                ...
                FcompanyName = orgDict.GetValueOrDefault(h?.FCompanyId ?? string.Empty, string.Empty),
        ...
        return new PagedResult<PurchaseOrderLabelListDto>
        {
            Items = items,
    ...
    private static PagedResult<PurchaseOrderLabelListDto> Empty(PagedRequest request) => new()
    {
        Items = new(),
        TotalCount = 0,
        PageIndex = request.PageIndex,
    ...
    public async Task<PurchaseOrderLabelHeadDto?> GetGenerateHeadAsync(string entryUid)
    {
        var entry = await _db.Queryable<TPurPoOrderEntry>().Where(e => e.Uid == entryUid && !e.FDeleted).FirstAsync();
        if (entry == null) return null;
        var header = await _db.Queryable<TPurPoOrder>().Where(h => h.Uid == entry.FInterId && !h.FDeleted).FirstAsync();
        if (header == null) return null;

        var dto = new PurchaseOrderLabelHeadDto
        {
            Uid = header.Uid,
            EntryUid = entry.Uid,
        ...
        dto.FcompanyName = await _db.Queryable<SysOrgStructure>().Where(o => o.Uid == header.FCompanyId).Select(o => o.Fname).FirstAsync() ?? string.Empty;
        ...
        var unit = await LoadUnitDictAsync(new[] { entry.Funitid });
    ...
    public async Task<List<BarcodeLineDto>> GenerateAsync(GenerateBarcodeRequest request)
    {
        var entry = await _db.Queryable<TPurPoOrderEntry>().Where(e => e.Uid == request.PoEntryUid && !e.FDeleted).FirstAsync()
            ?? throw new KeyNotFoundException("采购订单明细不存在");
        var header = await _db.Queryable<TPurPoOrder>().Where(h => h.Uid == entry.FInterId && !h.FDeleted).FirstAsync()
            ?? throw new KeyNotFoundException("采购订单不存在");
        if (header.FStatus != 40 || header.FDisabled)
            throw new InvalidOperationException("只有已审核且非禁用的采购订单才能打印标签");
        ...
        // 每个条码的数量 unitQty：
        //   单品条码(1)——恒为 1（忽略包装数量），共 ceil(打印数量) 个，如打印 100 → 100 个数量为 1 的条码；
        //   最小包装量(2)/批次(3)——等于包装数量，末个取余数，共 ceil(打印数量/包装数量) 个，如打印 120/包装 50 → 50,50,20。
        ...
        var companyId = string.IsNullOrEmpty(header.FCompanyId) ? (_currentUser.CompanyId ?? string.Empty) : header.FCompanyId;
        ...
        try
        {
            ...
            var barcodeNos = await _billCode.NextBarcodesAsync(BillCodeFormKeys.PurchaseOrder, count, ctx);
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                masters.Add(new TBdBarcoders
                {
                    ...
                    Fmfgdate = DateTime.MinValue, FDATE = null, FINSPECTDATE = null,
                    FCURRID = batchId,
                ...
                links.Add(new TBdBarcoders1
                {
                    ...
                    Fbarcode = barcodeNo,
                    Fpoid = header.Uid, Fpobillno = header.Fbillno,
                    Fpoentreyid = entry.FENTRYID ?? 0, Fpodetailid = entry.Uid,
                    Fpurid = string.Empty, Fpurbillno = string.Empty, Fpurentryid = 0, Fpurdetailid = string.Empty,
                    Fsupplierid = header.Fsupplyid,
                    Finspectid = string.Empty, Finspectdate = DateTime.MinValue,
                    FStatus = 0, FDeleted = false, FDisabled = false, FCompanyId = companyId,
                    CYmd = now, CUser = userId, MYmd = now, MUser = userId
                });
            }
    ...
    private async Task<Dictionary<string, (string Number, string Name, string Spec, decimal IncreaseQty, int KfPeriod)>> LoadMaterialDictAsync(IEnumerable<string> ids)
    {
        ...
        var rows = await _db.Queryable<TBdMaterial>().Where(m => list.Contains(m.Uid))
            .Select(m => new { m.Uid, m.FNumber, m.FName, m.FSpecification, m.FIncreaseQty, m.FKfPeriod }).ToListAsync();
        return rows.GroupBy(r => r.Uid).ToDictionary(g => g.Key,
            g => (g.First().FNumber, g.First().FName, g.First().FSpecification, g.First().FIncreaseQty, g.First().FKfPeriod));
    }
