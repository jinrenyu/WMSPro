  ...
  <div class="return-edit-container">
    ...
    <div class="edit-toolbar">
      <el-button type="primary" :disabled="isReadonly" @click="handleSave"
                 v-permission="isEdit ? 'purchasereturn:edit' : 'purchasereturn:add'">
        <el-icon><Check /></el-icon> 保存
      </el-button>
      <el-button v-if="isEdit && form.fStatus === 10" type="success" @click="handleApprove" v-permission="'purchasereturn:approve'">
        <el-icon><CircleCheck /></el-icon> 审核
      </el-button>
      <el-button v-if="isEdit && form.fStatus === 40" type="warning" @click="handleUnapprove" v-permission="'purchasereturn:approve'">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="84px"
        ...
        <el-row :gutter="16">
          <el-col :span="8">
            <el-form-item label="单据类型" prop="fbilltypeid">
              <LookupSelect v-model="form.fbilltypeid" module="billtype" parent-id="PUR_MRB" placeholder="请选择单据类型" preload />
            </el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <el-tabs v-model="activeTab" class="edit-tabs">
          ...
          <el-row :gutter="16">
            <el-col :span="6">
              <el-form-item label="退料类型">
                <el-select v-model="form.fmrtype" placeholder="退料类型" style="width:100%">
                  <el-option label="检验退料" value="A" />
                  <el-option label="库存退料" value="B" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="退料方式">
                <el-select v-model="form.fmrmode" placeholder="退料方式" style="width:100%">
                  <el-option label="退料补料" value="A" />
                  <el-option label="退料并扣款" value="B" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="库存组织">
                <el-select v-model="form.fcompanyid" placeholder="库存组织" style="width:100%">
          ...
          <el-row :gutter="16">
            <el-col :span="6">
              <el-form-item label="退料部门">
                <LookupSelect v-model="form.fmrdeptid" module="department" placeholder="请选择退料部门" preload />
              </el-form-item>
            </el-col>
            <el-col :span="6">
        ...
        <el-tab-pane label="其他" name="other">
          ...
          <el-row :gutter="16">
            <el-col :span="8"><el-form-item label="退料原因"><el-input v-model="form.fmrreason" placeholder="退料原因" /></el-form-item></el-col>
      ...
      <div class="scan-section">
        ...
        <el-input v-model="scanCode" class="scan-input" :disabled="form.ftypeid !== 2 || isReadonly"
                  :placeholder="form.ftypeid === 2 ? '扫描或输入已入库条码后回车' : '录入类型为「条码」时可用'"
                  @keyup.enter="onScan">
          <template #append>
        ...
        <el-tabs v-model="detailTab" class="detail-tabs">
          ...
          <el-tab-pane label="录入明细" name="barcode">
            <div class="grid-toolbar">
              <span class="grid-tip">扫码自动生成（仅已入库条码可退）；箱码保存时按装箱清单展开底阶条码</span>
              <div class="toolbar-spacer" />
          ...
          <el-tab-pane label="物料汇总明细" name="material">
            ...
            <el-table :data="materialLines" border size="small" row-key="_key" highlight-current-row
              ...
              <el-table-column label="实退数量" width="110">
                <template #default="{ row }">
                  <el-input-number v-model="row.frmrealqty" :min="0" :precision="4" :controls="false" :disabled="isReadonly || form.ftypeid === 2" style="width:100%" @change="() => recalcMaterial(row)" />
                </template>
              </el-table-column>
              <el-table-column prop="fcoefficient" label="换算率" width="90" align="right" />
...
import {
  getPurchaseReturn, createPurchaseReturn, updatePurchaseReturn,
  approvePurchaseReturn, unapprovePurchaseReturn, scanBarcode, getSourceBillTypes,
...
const newMaterialLine = () => ({
  ...
  frmrealqty: 0, freplenishqty: 0, fkeapamtqty: 0,
  funitid: '', funitNumber: '', funitName: '', fbaseunitid: '', fbaseunitqty: 0, fcoefficient: 1,
  fsecunitid: '', fsecunitqty: 0,
...
const recalcMaterial = (row: any) => {
  const qty = Number(row.frmrealqty) || 0
  ...
  for (const b of barcodeLines.value) {
    ...
    if (!g) {
      g = {
        ...
        fsecunitid: b.fsecunitid || '', fkfdate: b.fkfdate || null, fusefuldate: b.fusefuldate || null,
        ftaxprice: b.ftaxprice || 0, ftaxrate: b.ftaxrate || 0, fdiscountrate: b.fdiscountrate || 0,
        frmrealqty: 0, fsecunitqty: 0
      }
      map.set(k, g)
    }
    g.frmrealqty += Number(b.fqty) || 0
    g.fsecunitqty += Number(b.fsecunitqty) || 0
  }
  const lines = Array.from(map.values())
  lines.forEach(l => { recalcMaterial(l) })
...
async function loadDetail(id: string) {
  loading.value = true
  try {
    const res: any = await getPurchaseReturn(id)
    const d = res.data
    ...
    console.error('加载采购退料单失败:', e)
    ElMessage.error('加载采购退料单失败')
  } finally {
    loading.value = false
  }
}
...
function buildPayload() {
  const base: any = {
    ...
    fmrtype: form.fmrtype,
    fmrmode: form.fmrmode,
    freplenishmode: form.freplenishmode,
    fmrreason: form.fmrreason,
  ...
  if (form.ftypeid === 2) {
    base.barcodeEntries = barcodeLines.value.filter(it => it.fbarcode || it.fboxbarcode).map(it => ({
      ...
      fsecunitid: it.fsecunitid || '', fsecunitqty: it.fsecunitqty || 0, fsupplyid: it.fsupplyid || '',
    ...
    base.entries = materialLines.value.filter(it => it.fmaterialid).map(it => ({
      fmaterialid: it.fmaterialid, fauxpropid: it.fauxpropid || '', flot: it.flot || '',
      fstockid: it.fstockid || '', fstocklocid: it.fstocklocid || '', fstockstatusid: it.fstockstatusid || '',
      frmrealqty: it.frmrealqty || 0, freplenishqty: it.freplenishqty || 0, fkeapamtqty: it.fkeapamtqty || 0,
      funitid: it.funitid || '', fbaseunitid: it.fbaseunitid || '', fbaseunitqty: it.fbaseunitqty || 0, fcoefficient: it.fcoefficient || 1,
      fsecunitid: it.fsecunitid || '', fsecunitqty: it.fsecunitqty || 0,
      fkfdate: it.fkfdate || null, fusefuldate: it.fusefuldate || null,
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await updatePurchaseReturn(uid.value, buildPayload())
      ElMessage.success('保存成功')
      await loadDetail(uid.value)
    } else {
      const res: any = await createPurchaseReturn(buildPayload())
      ElMessage.success('创建成功')
      const newUid = res?.data?.uid
      if (newUid) {
        uid.value = newUid
        router.replace({ name: 'PurchaseReturnEdit', query: { uid: newUid } })
...
const handleApprove = () => runStatus(approvePurchaseReturn, '审核成功')
const handleUnapprove = () => runStatus(unapprovePurchaseReturn, '反审核成功')
const handleBack = () => router.push({ name: 'PurchaseReturnList' })
