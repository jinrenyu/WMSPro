  ...
  <div class="return-list-container">
    <div class="list-panel">
      ...
      <div class="toolbar-actions">
        <el-button type="primary" @click="handleAdd" v-permission="'purchasereturn:add'">
          <el-icon><Plus /></el-icon> 新增
        </el-button>
        <el-button @click="handleEditSelected" :disabled="selectedOrderIds.length !== 1" v-permission="'purchasereturn:edit'">
          <el-icon><Edit /></el-icon> 修改
        </el-button>
        <el-button type="success" @click="handleBatchApprove" :disabled="selectedOrderIds.length === 0" :loading="actionLoading" v-permission="'purchasereturn:approve'">
          审核{{ selectedOrderIds.length ? ` (${selectedOrderIds.length})` : '' }}
        </el-button>
        <el-button type="warning" @click="handleBatchUnapprove" :disabled="selectedOrderIds.length === 0" :loading="actionLoading" v-permission="'purchasereturn:approve'">
          反审核{{ selectedOrderIds.length ? ` (${selectedOrderIds.length})` : '' }}
        </el-button>
        <el-button type="danger" @click="handleBatchDelete" :disabled="selectedOrderIds.length === 0" :loading="actionLoading" v-permission="'purchasereturn:delete'">
...
const columns: ColumnDef[] = [
  ...
  { key: 'ftypeName', label: '录入类型', prop: 'ftypeName', width: 90, align: 'center', defaultVisible: false },
...
// 高级筛选仅开放可在服务端过滤的真实列（表头：单据编号/退料日期/数据状态；明细：实退数量/批次）
const filterColumns: ColumnDef[] = [
  { key: 'fbillno', label: '单据编号', prop: 'fbillno' },
  { key: 'fdate', label: '退料日期', prop: 'fdate' },
  { key: 'frmrealqty', label: '实退数量', prop: 'frmrealqty' },
  { key: 'flot', label: '批次', prop: 'flot' },
  { key: 'fStatus', label: '数据状态', prop: 'fStatus', slotName: 'status' },
]

const { allColumns, visibleKeys, configurableColumns, toggleColumn, resetColumns, isColumnVisible } = useColumnConfig('purchaseReturn', columns)
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getPurchaseReturns(queryParams)
...
const handleAdd = () => router.push({ name: 'PurchaseReturnEdit' })
const handleEdit = (uid: string) => router.push({ name: 'PurchaseReturnEdit', query: { uid } })
...
const handleBatchApprove = () => confirmBatch('审核', `确认审核选中的 ${selectedOrderIds.value.length} 张采购退料单？审核将退料出库并冲减库存。`, selectedOrderIds.value, approvePurchaseReturn)
const handleBatchUnapprove = () => confirmBatch('反审核', `确认反审核选中的 ${selectedOrderIds.value.length} 张采购退料单？`, selectedOrderIds.value, unapprovePurchaseReturn)
const handleBatchDelete = () => confirmBatch('删除', `确认删除选中的 ${selectedOrderIds.value.length} 张采购退料单？删除后不可恢复。`, selectedOrderIds.value, deletePurchaseReturn)
