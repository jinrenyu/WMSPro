  ...
  <div class="employee-list-container">
    ...
    <div class="header-actions">
      ...
      <div class="header-right">
        ...
        :columns="allColumns"
        :api-fields-func="getEmployeesFields"
        ...
        >
          <template v-if="col.slotName" #default="scope">
            <template v-if="col.slotName === 'sex'">
              {{ scope.row.fSex === 0 ? '男' : scope.row.fSex === 1 ? '女' : '' }}
            </template>
...
import { getEmployees, deleteEmployee, approveEmployee, unapproveEmployee, disableEmployee, enableEmployee, getEmployeesFields, type Employee } from '../../api/employee'
...
const handleAdd = () => { router.push({ name: 'EmployeeEdit', query: queryParams.groupId ? { groupId: queryParams.groupId } : {} }) }
const handleEdit = (row: Employee) => { router.push({ name: 'EmployeeEdit', query: { uid: row.uid } }) }
